...
import { updateData, setData } from "../firebase/firestore.js";
import { X_RapidAPI_Key, X_RapidAPI_Host } from "../env.js";
import { downloadPlayerImage } from '../firebase/storage.js';
import { fetchPlayerStats } from './fetchPlayerStats.js';
...
// club id in FootApi
const club_id_by_name =[42, 40, 60, 50, 30, 6, 38, 7, 48, 43, 44, 72, 17, 35, 39, 14, 15, 33, 37, 3]

// club id in FBREF
const FBREF_club_id_by_name = ['18bb7c10', '8602292d', '4ba7cbea', 'cd051869', 'd07537b9', '943e8050', 'cff3d9bb', '47c64c55', 'd3fd31cc', 'fd962109',
                            '822bd0ba', 'e297cd13', 'b8fd03ef', '19538871', 'b2b47a98', 'e4a775cb', '1df6b87e', '361ca564', '7c21e445', '8cec06e1']

const fetchPlayers = async (start_index, end_index) => {
    ...
    for (let i = start_index; i < end_index; i++) {
        ...
        console.log(`Time taken for club index ${i}: ${(func_end - func_start)/1000} s`);
    }
    // for player list
    // setData('player_list', 'full_player_list', {full_player_list});
    setData('player_list', `full_player_list(${start_index}-${end_index})`, {full_player_list});
    setData('player_list', `missing_statistics_player_list(${start_index}-${end_index})`, {missing_statistics_player_list});
    setData('player_list', `missing_statistics_or_name_translation_player_list(${start_index}-${end_index})`, {missing_statistics_or_name_translation_player_list});
    console.log(`Full player list (${start_index}-${end_index}) length:`, full_player_list.length);
    console.log(`Missing statistics player list (${start_index}-${end_index}) length:`, missing_statistics_player_list.length);
    console.log(`Missing statistics or name translation player list (${start_index}-${end_index}) length:`, missing_statistics_or_name_translation_player_list.length);
    const end = new Date();
    console.log(`Time taken for fetchPlayers (${start_index}-${end_index}): ${(end - start)/1000} s`);
}
