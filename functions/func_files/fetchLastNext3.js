...
const fetchLast3 = async (team_id, team_dataClubs) => {
    ...
    for (let i = result.events.length-1; i >= 0; i--) {
        ...
        if (match.tournament.id === 1 && last3_array.length < 3) {
            ...
            if (match.winnerCode === 3) {   // draw
                ...
                if (team_dataClubs.club.name_code === match.homeTeam.nameCode) {
                    result = 'Win';
                } else {
                    result = 'Loss';
                }
            } else if (match.winnerCode === 2) {    // away team won
                if (team_dataClubs.club.name_code === match.awayTeam.nameCode) {
                    result = 'Win';
                } else {
                    result = 'Loss';
                }
...
const fetchNext3 = async (team_id, database_id) => {
    ...
    for (let i = 0; i <= result.events.length-1; i++) {
        ...
        if (match.tournament.id === 1 && next3_array.length < 3) {
            ...
            next3_array.push({
                date: `${match_day}, ${matchdate.getDate()} ${match_month}`,
                time: moment(matchdate).utcOffset('+0800').format('HH:mm'),
...
const fetchLastNext3 = async () => {
    ...
    const mid = new Date();
    console.log("Time taken for fetchLastNext3 (get dataClubs): ", (mid - start) / 1000, "s");

    for (let i = 0; i < club_id_by_name.length; i++) {
        const club_start = new Date();
        const last3 = await fetchLast3(club_id_by_name[i], dataClubs[i]);
        const next3 = await fetchNext3(club_id_by_name[i], dataClubs[i]);
        ...
        const club_end = new Date();
        console.log(`Time taken for fetchLastNext3 (${dataClubs[i].club.name_code}): `, (club_end - club_start) / 1000, "s");
    }

    const end = new Date();
    console.log("Time taken for fetchLastNext3: ", (end - start) / 1000, "s");
}
